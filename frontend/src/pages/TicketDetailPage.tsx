...
function fieldLabel(field: string): string {
  return { status: "Status", priority: "Priority", assignee_id: "Assignee", epic_id: "Epic" }[field] ?? field;
}
    ...
    <div className="ticket-detail">
      ...
      <div className="ticket-detail-fields">
        ...
        <div className="ticket-assignee-field">
          <span>Assignee: {ticket.assignee ? ticket.assignee.username : "Unassigned"}</span>
          {user && ticket.assignee?.id === user.id ? (
            <button type="button" className="btn-ghost" onClick={() => patch({ assignee_id: null })}>
              Unassign
            </button>
          ) : (
            <button type="button" className="btn-ghost" onClick={() => patch({ assignee_id: user?.id })}>
              Assign to me
            </button>
          )}
        </div>
      ...
      <p className="ticket-reporter">Reported by {ticket.reporter.username}</p>
      ...
      <ul className="ticket-comment-list">
        ...
        {ticket.comments.map((comment) => (
          <li key={comment.id} className="ticket-comment-row">
            <span className="ticket-comment-author">{comment.author.username}</span>
            <p>{comment.body}</p>
          </li>
        ))}
      </ul>
      ...
      <ul className="ticket-activity-list">
        ...
        {ticket.activity.map((entry) => (
          <li key={entry.id} className="ticket-activity-row">
            {entry.user.username} changed {fieldLabel(entry.field)} from{" "}
